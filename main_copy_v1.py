...
def setup_logging():
    ...
    root.info(f"Models: Deepgram STT (nova-3, hi - Hindi-first with English code-mixing), Groq LLM (llama-3.3-70b-versatile), ElevenLabs TTS (flash_v2_5)")
    root.info(f"=" * 80)
...
# Best model for 100% accurate, complete, natural translations
GROQ_LLM = openai.LLM(
...
CONTEXT_WINDOW_SIZE = 5  # 5 messages: Reference context for pronoun resolution (prevents context bleeding with clear separation)
CONTEXT_MAX_MESSAGES = 7  # System(1) + Previous 5 messages(5) + Current(1) = 7 - Reference context for quality
CONTEXT_MAX_ITEMS = 12  # System(1) + Previous 5 user messages(5) + Current(1) = 7 items max (prevents bleeding)
# ⚠️ ARCHITECTURAL DESIGN: REFERENCE CONTEXT MODE
# Context includes previous 5 messages but CLEARLY MARKED as "REFERENCE ONLY - DO NOT TRANSLATE"
# Strategy: Include previous messages for pronoun resolution, but explicitly instruct LLM to translate ONLY current message
# Benefit: Pronoun resolution + Zero context bleeding (clear separation prevents merging)
# This approach provides best quality (100% accurate) while preventing hallucination
...
# STT - Deepgram Nova-3 (Best Accuracy for Hindi/Hinglish) - Optimized for Maximum Accuracy
# CRITICAL FIX: Using language="hi" (NOT "multi") to prevent Hindi words being transcribed as English
# Research: language="hi" is optimized for Hindi-first code-mixed speech (Hinglish) and handles English words naturally
# Problem: "multi" has English bias - "बीच" (middle) was being transcribed as "beach" (English)
# Solution: language="hi" prioritizes Hindi recognition while still supporting English words in Hinglish
# Common Hindi words that are often misrecognized as English - add to keyterms for better recognition
HINDI_KEYTERMS = [
    "बीच",      # middle (often misrecognized as "beach")
    "में",       # in/at (common word)
    "क्या",      # what (common word)
    "है",        # is (common word)
    "हैं",       # are (common word)
    "तो",        # so/then (common word)
    "तू",        # you (informal)
    "तेरे",      # your (informal)
    "मेरे",      # my
    "दोस्त",     # friend
...
DEEPGRAM_STT = deepgram.STT(
    model="nova-3",  # ⬆️ UPGRADED: 27% WER reduction for Hindi, superior Hinglish support, better accuracy
    language="hi",  # ⬆️ CRITICAL FIX: "hi" (NOT "multi") - Optimized for Hindi-first with English code-mixing support
    # Why "hi" not "multi": "multi" has English bias, "hi" correctly recognizes Hindi words like "बीच" (middle) not "beach"
    # Deepgram's "hi" language setting is designed for code-mixed speech (Hinglish) - handles English words naturally
    keyterms=HINDI_KEYTERMS,  # ⬆️ ADDED: Keyterms to improve recognition of common Hindi words (prevents "बीच"→"beach")
    smart_format=True,  # ⬆️ ENABLED: Better punctuation and accuracy - helps LLM understand context better
    endpointing_ms=STT_ENDPOINTING_MS,  # 220ms pause detection - Balanced for accuracy and latency
    ...
    # CRITICAL FIXES:
    # - nova-3: Best accuracy model (27% better than nova-2 for Hindi)
    # - language="hi": Hindi-first recognition with English code-mixing support (fixes "बीच"→"beach" issue)
    # - keyterms: Common Hindi words to prevent misrecognition (especially "बीच" not "beach")
    # - smart_format=True: Better punctuation improves LLM understanding and reduces errors
    # This configuration maximizes STT accuracy for Hindi/Hinglish speech while preventing English bias
)
...
TRANSLATION_INSTRUCTIONS = (
    "You are a professional Hindi/Hinglish-to-English translator for live streaming. Your job is 100% accurate, complete, natural, human-like translation.\n\n"
    
    "CRITICAL OUTPUT RULES:\n"
    "1. OUTPUT LANGUAGE: STRICTLY ENGLISH ONLY - Never output Hindi, Devanagari, or any non-English text\n"
    "2. COMPLETE TRANSLATION: Translate EVERY word - do NOT cut, skip, or omit any words from the input\n"
    "3. EXACT BUT NATURAL: Translate word-for-word accurately, but make it sound natural and human-like\n"
    "4. ENGLISH INPUT: If input is already in English, output it AS-IS (do not translate English to English)\n"
    "5. NO WORD CUTTING: Ensure complete sentences - never truncate or cut words mid-sentence\n\n"
    
    "FEW-SHOT EXAMPLES (Learn from these patterns):\n"
    "Example 1:\n"
    "  Input: \"नमस्ते, जय श्री राम भाई\"\n"
    "  Output: \"Namaste, jai shri ram brother\"\n"
    "  Note: Religious terms stay in Hindi transliteration, casual tone preserved\n\n"
    "Example 2:\n"
    "  Input: \"Nifty ऊपर जा रहा है\"\n"
    "  Output: \"Nifty is going up\"\n"
    "  Note: English words (Nifty) kept as-is, Hindi translated naturally\n\n"
    "Example 3:\n"
    "  Input: \"हां जी pre market report डालो\"\n"
    "  Output: \"Yes, put the pre market report\"\n"
    "  Note: Respectful \"जी\" → \"Yes\", English terms kept, natural sentence structure\n\n"
    "Example 4:\n"
    "  Input: \"मैं यहां हूं भाई, Telegram channel पर देखा?\"\n"
    "  Output: \"I am here brother, did you see me on the Telegram channel?\"\n"
    "  Note: Casual \"भाई\" → \"brother\", complete sentence with proper grammar\n\n"
    "Example 5:\n"
    "  Input: \"Welcome welcome आज late है\"\n"
    "  Output: \"Welcome welcome, today is late\"\n"
    "  Note: English pass-through, Hindi added naturally, casual tone\n\n"
    
    "CONTEXT USAGE (Previous Messages):\n"
    "• Previous messages are provided as separate messages for context\n"
    "• Use them ONLY for pronoun resolution (यह/वह/उसका/इसका/उनका)\n"
    "• DO NOT translate previous messages - they are for reference only\n"
    "• DO NOT merge, continue, or combine previous messages with current translation\n"
    "• Translate ONLY the CURRENT message (marked as [Translate])\n\n"
    
    "PRONOUN RESOLUTION GUIDE:\n"
    "• यह (this) → Resolve from previous messages (e.g., \"यह\" → \"Nifty\" if previous was about Nifty)\n"
    "• वह (that) → Resolve from previous messages (e.g., \"वह\" → \"the market\" if previous was about market)\n"
    "• उसका/इसका (his/its) → Resolve from previous context\n"
    "• उनका (their) → Resolve from previous context\n"
    "• If pronoun referent is unclear, use generic: \"यह\" → \"it\", \"वह\" → \"that\"\n\n"
    
    "TRANSLATION QUALITY REQUIREMENTS:\n"
    "1. ACCURACY: Translate every single word accurately - \"और\"=and, \"भी\"=also, \"तो\"=so, \"नहीं\"=not\n"
    "2. COMPLETENESS: Never cut words - if input has 10 words, output must have equivalent 10+ words\n"
    "3. NATURALNESS: Make it sound like natural human speech, not robotic literal translation\n"
    "4. GRAMMAR: Complete sentences with proper subjects - \"जा रहा है\" → \"It is going\" (not \"is going\")\n"
    "5. TENSE PRESERVATION: है=is, था=was, गा=will - preserve exactly\n"
    "6. HINGLISH HANDLING: Keep English words as-is - Nifty, Telegram, channel, subscribe, etc.\n"
    "7. RELIGIOUS TERMS: Keep in transliterated Hindi - \"नमस्ते\"→\"namaste\", \"जय श्री राम\"→\"jai shri ram\"\n"
    "8. TONE PRESERVATION: \"भाई\" (brother) → casual, \"जी\" (sir) → respectful, \"!\" → excited\n"
    "9. INCOMPLETE INPUT: If input ends with \"...\", translate what's there naturally, don't add context\n\n"
    
    "ENGLISH INPUT DETECTION:\n"
    "• If input contains ONLY English (no Hindi/Devanagari), output it AS-IS with proper formatting\n"
    "• Example: Input \"Hello, how are you?\" → Output \"Hello, how are you?\" (NOT translated)\n"
    "• Example: Input \"Nifty is going up\" → Output \"Nifty is going up\" (NOT translated)\n\n"
    
    "TTS OPTIMIZATION:\n"
    "• Use punctuation for natural rhythm: periods for stops, commas for pauses, ? for questions\n"
    "• Numbers: 25→\"twenty-five\", ₹500→\"five hundred rupees\"\n"
    "• Remove fillers (acha, toh) unless they add emotion\n"
    "• Short, clear sentences for better TTS flow\n\n"
    
    "OUTPUT FORMAT:\n"
    "• Plain English text ONLY\n"
    "• NO Hindi, NO Devanagari, NO explanations, NO markdown, NO quotes\n"
    "• NO code, NO numbers, NO structured data, NO JSON, NO XML\n"
    "• NO repeated patterns, NO nonsense words, NO technical terms unless in input\n"
    "• Complete sentences - never cut words or truncate\n"
    "• Natural human-like speech\n"
    "• Keep output SHORT - translate only what is in the input, nothing more\n\n"
    
    "CRITICAL: STOP GENERATING IMMEDIATELY AFTER TRANSLATION\n"
    "• Once you've translated the input, STOP. Do NOT continue generating.\n"
    "• Do NOT generate code, data structures, or any additional content.\n"
    "• Do NOT repeat words or patterns.\n"
    "• Output should be 1-3 sentences maximum for typical input.\n"
)
...
def sanitize_text_for_tts(text: str) -> str | None:
    """Sanitize text for TTS - ensures English-only output, removes any Hindi that might leak through"""
...
def prune_chat_context(chat_ctx: llm.ChatContext, logger_name: str = 'llm') -> int:
    ...
    # ========================================================================
    # Goal: Keep context within CONTEXT_MAX_ITEMS (12) to prevent:
    ...
    #
    # Example with CONTEXT_MAX_ITEMS=12:
    #   Before: [system, user1, asst1, user2, asst2, ..., user10, asst10] (21 items)
    #   After:  [system, user6, user7, user8, user9, user10] (6 items - only user messages)
    #   Room for: new user message (7 items total, but reference context used for LLM)
    # ========================================================================
    ...
    if system_msg:
        ...
        # Keep last 5 user messages for reference context (pronoun resolution)
        keep_count = 5  # Keep last 5 user messages for reference
    ...
    try:
        ...
        for attempt in range(max_stt_retries):
            try:
                ...
                LOGGERS['stt'].info(f"Deepgram config - Model: nova-3, Language: hi (Hindi-first with English code-mixing), Keyterms: {len(HINDI_KEYTERMS)} Hindi words, Endpointing: {STT_ENDPOINTING_MS}ms, VAD: enabled, Smart Format: enabled")
                        ...
                        async for event in stt_stream:
                            if event.type == stt.SpeechEventType.START_OF_SPEECH:
                                ...
                                if event.alternatives and event.alternatives[0].text.strip():
                                    ...
                                    # CRITICAL FIX: Post-process to correct common misrecognitions
                                    # Fix "beach" → "बीच" (middle) when it appears in Hindi context
                                    # Check if text contains Hindi characters (indicating Hindi context)
                                    has_hindi = bool(re.search(r'[\u0900-\u097F]', text))
                                    if has_hindi:
                                        # In Hindi context, replace "beach" with "बीच" (middle)
                                        # Use word boundaries to avoid replacing "beach" in compound words
                                        original_text = text
                                        text = re.sub(r'\bbeach\b', 'बीच', text, flags=re.IGNORECASE)
                                        # Log if correction was made
                                        if original_text != text:
                                            LOGGERS['stt'].info(f"✅ Post-processed: Fixed 'beach' → 'बीच' in Hindi context")
...
async def process_single_translation(hindi_text: str, chat_ctx: llm.ChatContext,
    ...
    # ============================================================================
    # CRITICAL FIX #1: Input Validation & English Detection BEFORE LLM Call
    # ============================================================================
    ...
    # Detect if input is pure English (no Hindi/Devanagari characters)
    # Devanagari Unicode range: U+0900 to U+097F
    has_hindi = bool(re.search(r'[\u0900-\u097F]', hindi_text))
    is_pure_english = not has_hindi and len(hindi_text.split()) > 0
    
    if is_pure_english:
        # Pure English input - return as-is with proper formatting
        LOGGERS['llm'].info(f"✅ Pure English input detected - passing through as-is: '{hindi_text[:100]}...'")
        # Clean up formatting but keep content
        english_output = hindi_text.strip()
        # Ensure proper sentence ending
        if english_output and not english_output[-1] in '.!?':
            english_output += '.'
        return english_output
    
    
    # ============================================================================
    # CRITICAL FIX #2: REFERENCE CONTEXT - Best Quality with Zero Bleeding
    # ============================================================================
    # Create context with previous 5 messages for pronoun resolution, but CLEARLY MARKED
    # as reference only to prevent context bleeding
    # Strategy: System + Previous 5 messages (marked as reference) + Current message
    async with translation_lock:
        # Prune main context first to keep only last 5 user messages
        prune_chat_context(chat_ctx, logger_name='llm')
        
        # Create translation context with system + previous messages (reference) + current
        ...
        if system_msg:
            # Ensure system_msg.content is a string
            system_content = system_msg.content
            if isinstance(system_content, list):
                system_content = ' '.join(str(item) for item in system_content)
            elif not isinstance(system_content, str):
                system_content = str(system_content) if system_content else TRANSLATION_PROMPT
            translation_ctx.add_message(role="system", content=system_content)
        else:
            # Fallback: add system prompt directly
            translation_ctx.add_message(role="system", content=TRANSLATION_PROMPT)
        
        # Get previous 5 user messages for reference (pronoun resolution)
        user_messages = [item for item in chat_ctx.items if item.type == "message" and item.role == "user"]
        previous_messages = user_messages[-5:] if len(user_messages) > 0 else []
        
        # IMPROVED CONTEXT STRUCTURE: Separate messages for better LLM processing
        # Strategy: Each previous message as separate user message (for context)
        # Then current message clearly marked (for translation)
        # This helps LLM understand conversation flow better
        
        # Add previous messages as separate context messages (for pronoun resolution)
        if previous_messages:
            # Build pronoun resolution hints from previous messages
            pronoun_hints = []
            for i, msg in enumerate(previous_messages, 1):
                # Ensure msg.content is a string (handle case where it might be a list or other type)
                ...
                # Add as separate message for better context understanding
                translation_ctx.add_message(role="user", content=f"[Context {i}] {msg_content}")
                
                # Extract potential pronoun referents (entities mentioned)
                # Simple heuristic: Capitalized words, numbers, or common entities
                try:
                    ...
                    for w in words:
                        if len(w) > 0:
                            try:
                                if (w[0].isupper() and len(w) > 2) or w.isdigit() or w.lower() in ['nifty', 'telegram', 'market', 'report']:
                                    entities.append(w)
                            except (IndexError, AttributeError):
                                # Skip word if it causes an error
                                continue
                    if entities:
                        pronoun_hints.extend(entities[:3])  # Keep top 3 entities per message
                except (AttributeError, IndexError, TypeError) as e:
                    # Skip entity extraction if there's an error
                    LOGGERS['llm'].debug(f"Could not extract entities from message {i}: {str(e)}")
                    continue
            
            # Add pronoun resolution summary if we have hints
            if pronoun_hints:
                unique_entities = list(dict.fromkeys(pronoun_hints))[:5]  # Remove duplicates, keep first 5
                pronoun_summary = f"[Pronoun Reference] Recent entities mentioned: {', '.join(unique_entities)}. Use these to resolve pronouns (यह/वह/उसका) in the current message."
                translation_ctx.add_message(role="user", content=pronoun_summary)
        
        # Add current message to translate (clearly marked)
        translation_ctx.add_message(role="user", content=f"[Translate] {hindi_text}")
        
        # Update main context for tracking
                ...
                async with tts_stream:
                    ...
                    try:
                                ...
                                async with MAIN_LLM.chat(chat_ctx=translation_ctx) as llm_stream:
                                    async for chunk in llm_stream:
                                        ...
                                        if text:
                                            ...
                                            # CRITICAL: Early termination for hallucination detection
                                            # If LLM generates too many chunks or suspicious patterns, stop immediately
                                            current_length = len(full_translation)
                                            
                                            # Check 1: Excessive chunk count (likely hallucination)
                                            # For 28-char input, 100 chunks is already suspicious (should be ~10-20)
                                            max_reasonable_chunks = max(50, input_length * 2)  # At most 2 chunks per char
                                            if llm_chunk_count > max_reasonable_chunks:
                                                LOGGERS['llm'].error(f"🚨 HALLUCINATION DETECTED: {llm_chunk_count} chunks for {input_length}-char input (max: {max_reasonable_chunks}) - stopping stream")
                                                LOGGERS['llm'].error(f"🚨 Output so far: '{full_translation[:200]}...'")
                                                break
                                            
                                            # Check 2: Excessive output length (likely hallucination)
                                            # Output should not exceed 10x input length (even for complex translations)
                                            max_reasonable_length = input_length * 10
                                            if current_length > max_reasonable_length:
                                                LOGGERS['llm'].error(f"🚨 HALLUCINATION DETECTED: {current_length} chars output for {input_length}-char input (max: {max_reasonable_length}) - stopping stream")
                                                LOGGERS['llm'].error(f"🚨 Output so far: '{full_translation[:200]}...'")
                                                break
                                            
                                            # Check 3: Suspicious patterns (code-like, numbers, repeated patterns)
                                            # Detect patterns like "439439439", "Model439", "ItemId439" (garbage from log)
                                            suspicious_patterns = [
                                                r'\d{3,}\d{3,}',  # Repeated 3+ digit numbers (e.g., "439439439")
                                                r'[A-Z][a-z]+\d{3,}',  # Word followed by 3+ digits (e.g., "Model439")
                                                r'ItemId\d+|File\d+|Style\d+',  # Code-like patterns
                                                r'Hoover\d+|Zot\d+|Mode\d+',  # Nonsense patterns from log
                                            ]
                                            recent_text = full_translation[-100:] if len(full_translation) > 100 else full_translation
                                            pattern_found = False
                                            for pattern in suspicious_patterns:
                                                if re.search(pattern, recent_text):
                                                    LOGGERS['llm'].error(f"🚨 HALLUCINATION DETECTED: Suspicious pattern '{pattern}' found in output - stopping stream")
                                                    LOGGERS['llm'].error(f"🚨 Output so far: '{full_translation[:200]}...'")
                                                    pattern_found = True
                                                    break
                                            
                                            # If suspicious pattern found, stop processing this chunk
                                            if pattern_found:
                        ...
                        if translation_success:
                            # Final validation: Check if output is suspicious even after completion
                            if len(full_translation) > input_length * 10:
                                LOGGERS['llm'].error(f"🚨 CRITICAL: Output length {len(full_translation)} chars is {len(full_translation)/input_length:.1f}x input ({input_length} chars) - likely hallucination")
                                LOGGERS['llm'].error(f"🚨 Truncating to reasonable length: {input_length * 5} chars")
                                original_length = len(full_translation)
                                full_translation = full_translation[:input_length * 5].strip()
                                # Try to end at a sentence boundary
                                last_period = full_translation.rfind('.')
                                last_exclamation = full_translation.rfind('!')
                                last_question = full_translation.rfind('?')
                                last_sentence_end = max(last_period, last_exclamation, last_question)
                                if last_sentence_end > input_length * 3:  # At least 60% of truncated length
                                    full_translation = full_translation[:last_sentence_end + 1]
                                LOGGERS['llm'].warning(f"🚨 Truncated from {original_length} to {len(full_translation)} chars due to hallucination")
                            
                            # If translation was stopped early due to hallucination, use only the first reasonable sentence
                            if len(full_translation) > input_length * 5:
                                # Find first complete sentence
                                first_period = full_translation.find('.')
                                first_exclamation = full_translation.find('!')
                                first_question = full_translation.find('?')
                                first_sentence_end = min(
                                    x for x in [first_period, first_exclamation, first_question] if x > 0
                                ) if any(x > 0 for x in [first_period, first_exclamation, first_question]) else -1
                                
                                if first_sentence_end > 0 and first_sentence_end < input_length * 3:
                                    LOGGERS['llm'].warning(f"🚨 Using only first sentence due to excessive output: '{full_translation[:first_sentence_end+1]}'")
                                    full_translation = full_translation[:first_sentence_end + 1]
    ...
    # CRITICAL: Final validation - Ensure no Hindi leaked through
    # Remove any Hindi/Devanagari characters that might have leaked
    hindi_in_output = re.findall(r'[\u0900-\u097F]+', cleaned_translation)
    if hindi_in_output:
        LOGGERS['llm'].error(f"🚨 CRITICAL: Hindi detected in output! Removing: '{''.join(hindi_in_output)}'")
        cleaned_translation = re.sub(r'[\u0900-\u097F]+', '', cleaned_translation).strip()
        if not cleaned_translation:
            LOGGERS['llm'].error("🚨 Output became empty after removing Hindi - this is a critical error")
            return ""
    ...
    # Ensure complete translation - check word count ratio
    input_words = len(hindi_text.split())
    output_words = len(cleaned_translation.split())
    if input_words > 0 and output_words > 0:
        word_ratio = output_words / input_words
        if word_ratio < 0.3:  # Output has less than 30% of input words - likely incomplete
            LOGGERS['llm'].warning(f"⚠️ Translation may be incomplete: Input {input_words} words → Output {output_words} words (ratio: {word_ratio:.2f})")
        elif word_ratio > 0.3 and word_ratio < 0.6:  # 30-60% - might be cutting words
            LOGGERS['llm'].warning(f"⚠️ Translation might be cutting words: Input {input_words} words → Output {output_words} words (ratio: {word_ratio:.2f})")
        
    LOGGERS['llm'].info(f"Translation completed - Result: '{cleaned_translation[:100]}{'...' if len(cleaned_translation) > 100 else ''}' ({len(cleaned_translation)} chars, {output_words} words, expansion: {expansion_ratio:.2f}x)")
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    LOGGERS['main'].info(f"  - STT: Deepgram Nova-3 (hi - Hindi-first) - Best accuracy for Hindi/Hinglish, prevents English bias (27% WER reduction)")
